...
\title{Phase-Space Seams: Invariant Volume Corrections and Universal Positivity at All Even Jet Orders}
...
\begin{abstract}
We introduce \emph{phase-space seams}, a scalar-first framework in which a real-valued
scalar field $s$ on $\mathbb{R}^{2d}$ generates geometric data via a Hessian rule and a
family of sublevel sets. Our main result is an invariant first-variation formula in
$\mathbb{R}^n$: to first order, the volume of small sublevel sets $\{s\le\varepsilon\}$
depends on the quartic jet only through the unique $O(n)$-scalar double trace of the
4-jet tensor; normalisability of the model density $\exp(-s_4)$ forces positivity of the
correction coefficient. For the quadratic seam
$s(x,p)=x^2\!/(2\sigma_x^2)+p^2\!/(2\sigma_p^2)$ the sublevel set $\{s\le 1/2\}$ is an
ellipse of Lebesgue area $\pi\sigma_x\sigma_p$. Retaining the full fourth-order Taylor
jet, including the cross-term $x^2p^2$, yields a coordinate-invariant first-order area
correction in which the cross-coupling contributes with coefficient $1/8$, distinct from
the diagonal coefficient $3/16$, reflecting universal angular integrals. We extend the
framework to the sextic jet and prove a general first-variation formula for arbitrary
even-order $2k$-jets in $\mathbb{R}^n$. Universality holds at all even orders: for any
non-trivial normalisable $2k$-jet perturbation the $k$-fold trace of the jet tensor is
strictly positive, so non-Gaussianity always reduces sublevel-set volumes to first order;
the proof is a single measure-theoretic step---spherical averaging of a non-negative
function---subsuming the quartic AM--GM argument. Finally, for the quartic model density
$\rho\propto\exp(-s_4)$ we compute true second moments to first order in the quartic
strength~$\lambda$, making explicit that the jet parameters $\sigma_x$, $\sigma_p$
generally differ from the moments of $\rho$ in non-Gaussian models.
\end{abstract}

\keywords{Hessian geometry, phase-space distributions, sublevel sets, non-Gaussianity, information geometry, uncertainty relations, jet calculus, spherical averaging}
...
\emph{seam}) and let an explicit local rule generate the geometric data. We use the term
``seam'' to evoke the stitching of local geometric data into a global fabric, while
remaining compatible with the log-density/potential-function language of information
geometry. In this note we apply this scalar-first philosophy to classical phase space
\(\mathbb{R}^{2d}\) equipped with the standard symplectic coordinates \((x,p)\).
...
\end{equation}
The diagonal quartic seam corresponds to the restriction \(a = b = 1\),
...
For sufficiently small \(\varepsilon>0\), the sublevel set \(\{s\le \varepsilon\}\) is star-shaped about the
origin, so its boundary can be written as \(\{r(n)\,n: n\in S^{n-1}\}\) for a smooth radial function
\(r(n)>0\). With \(g=I\), write \(z=rn\) with \(n\in S^{n-1}\). The jet expansion gives
...
\medskip
\begin{table}[tb]
\caption{The first levels of the even-jet hierarchy of conditional width-product bounds.
At order \(2k\), the jet data define a symmetric \(2k\)-tensor whose \(k\)-fold trace \(\mathcal{W}_{2k}\)
governs the leading correction \(1+\mathcal{W}_{2k}+\cdots\); normalisability forces \(\mathcal{W}_{2k}>0\) for non-trivial perturbations.}
\label{tab:jet-hierarchy}
\centering
\begin{tabular}{clll}
\hline
...
4 & \(T_{ijkl}\), double trace \(\mathcal{W}_4\)
  & \(\sigma_x\sigma_p \ge (\hbar/2)(1 + \mathcal{W}_4 + \cdots)\)
  & Positive iff normalisable \\
6 & \(S_{ijklmn}\), triple trace \(\mathcal{W}_6\)
  & \(\sigma_x\sigma_p \ge (\hbar/2)(1 + \mathcal{W}_6 + \cdots)\)
  & Positive iff normalisable \\
\(2k\) & Symmetric \(2k\)-tensor, \(k\)-fold trace \(\mathcal{W}_{2k}\)
...
AM-GM inequality and the normalisability constraint (Theorem~\ref{thm:universality}).
The general statement --- that normalisability forces positivity at every even order
\(2k\) --- is established in Theorem~\ref{thm:universality-all-orders} via a
spherical-averaging argument that subsumes the quartic AM-GM proof as a special case.
...
%------------------------------------------------------------------
\section{The Sextic Jet: Extending the Hierarchy}
\label{sec:sextic}
%------------------------------------------------------------------

The quartic computation of Section~\ref{sec:fourjet} extracted the first-order sublevel-set
correction from the 4-jet. We now carry out the analogous computation for the \emph{6-jet},
establishing the next level of the hierarchy announced in Section~\ref{sec:hierarchy}.
As before, we give explicit formulas in \(d=1\) and then state the coordinate-free result
in arbitrary dimension.

%------------------------------------------------------------------
\subsection{The Pure Sextic Seam in Dimension \texorpdfstring{$d=1$}{d=1}}
\label{subsec:sextic-seam}
%------------------------------------------------------------------

In normalised coordinates \(u=x/\sigma_x\), \(v=p/\sigma_p\), define the \emph{pure sextic
seam} (with vanishing quartic jet, for clarity; combined effects are additive at leading
order):
\begin{equation}
s_6(u,v) = \frac{u^2+v^2}{2}
  + \frac{\mu}{6}\bigl(\alpha\,u^6 + \beta\,v^6
    + 3\gamma\,u^4 v^2 + 3\delta\,u^2 v^4\bigr),
\label{eq:sextic-seam}
\end{equation}
where \(\alpha,\beta\ge 0\) and \(\gamma,\delta\in\mathbb{R}\) are dimensionless shape
parameters and \(\mu>0\) sets the overall strength of the sextic correction. In polar
coordinates \(u=r\cos\theta\), \(v=r\sin\theta\), this becomes
\begin{equation}
s_6 = \frac{r^2}{2} + \frac{\mu}{6}\,r^6\,G(\theta),
\label{eq:sextic-polar}
\end{equation}
where
\begin{equation}
G(\theta) := \alpha\cos^6\theta + \beta\sin^6\theta
  + 3\gamma\cos^4\theta\sin^2\theta + 3\delta\cos^2\theta\sin^4\theta.
\label{eq:G-def}
\end{equation}

%------------------------------------------------------------------
\subsection{Sextic Angular Integrals and Area Correction}
\label{subsec:sextic-area}
%------------------------------------------------------------------

The four angular integrals needed are:
\begin{align}
\int_0^{2\pi}\cos^6\theta\,d\theta
  &= \int_0^{2\pi}\sin^6\theta\,d\theta = \frac{5\pi}{8},
\label{eq:int-sextic-diag}\\
\int_0^{2\pi}\cos^4\theta\sin^2\theta\,d\theta
  &= \int_0^{2\pi}\cos^2\theta\sin^4\theta\,d\theta = \frac{\pi}{8}.
\label{eq:int-sextic-cross}
\end{align}
The ratio of cross to diagonal integrals is \(1/5\), compared with \(1/3\) at quartic
order~\eqref{eq:int-diag}--\eqref{eq:int-cross}. Both are instances of a general
pattern: at order \(2k\), the ratio is \(1/(2k-1)\).

Therefore,
\begin{equation}
\int_0^{2\pi}G(\theta)\,d\theta
= \frac{\pi}{8}\bigl(5\alpha + 5\beta + 3\gamma + 3\delta\bigr).
\label{eq:G-integral}
\end{equation}

The boundary of \(\{s_6=1/2\}\) satisfies
\(\tfrac{r^2}{2}+\tfrac{\mu}{6}r^6 G(\theta)=\tfrac12\). Solving perturbatively gives
\begin{equation}
r^2(\theta) = 1 - \frac{\mu}{3}\,G(\theta) + O(\mu^2).
\label{eq:r2-sextic}
\end{equation}

The area in normalised coordinates is therefore
\begin{align}
A_{uv} &= \frac{1}{2}\int_0^{2\pi}r^2(\theta)\,d\theta
= \pi - \frac{\mu}{6}\cdot\frac{\pi}{8}(5\alpha+5\beta+3\gamma+3\delta) + O(\mu^2)
\notag\\
&= \pi\!\left(1 - \frac{\mu(5\alpha+5\beta+3\gamma+3\delta)}{48}\right) + O(\mu^2).
\label{eq:sextic-area-normalised}
\end{align}

Restoring physical units:
\begin{equation}
\boxed{
A(s_6) = \pi\sigma_x\sigma_p\!\left(1
  - \frac{\mu(5\alpha+5\beta+3\gamma+3\delta)}{48}\right) + O(\mu^2).
}
\label{eq:sextic-area-physical}
\end{equation}

\begin{remark}[Angular-integral ratio \(1/5\) at sextic order]
Each diagonal shape parameter \(\alpha\) or \(\beta\) contributes with the angular weight
\(5\pi/8\), while each cross parameter \(\gamma\) or \(\delta\) contributes with \(\pi/8\).
Their ratio of \(1/5\) is the sextic analogue of the quartic ratio \(1/3\) and reflects
the universal spherical-geometry identity
\(\int_0^{2\pi}\cos^4\theta\sin^2\theta\,d\theta \,/\,
\int_0^{2\pi}\cos^6\theta\,d\theta = 1/5\).
\end{remark}

%------------------------------------------------------------------
\subsection{Invariant First Variation at General Even Order}
\label{subsec:general-first-variation}
%------------------------------------------------------------------

The quartic first variation (Theorem~\ref{thm:invariant-first-variation}) and the sextic
computation above are special cases of a general first-variation formula for the pure
\(2k\)-jet, valid in arbitrary dimension~\(n\).

\begin{theorem}[First variation from the \(2k\)-jet]
\label{thm:general-2k-variation}
Let \(s:\mathbb{R}^n\to\mathbb{R}\) be \(C^{2k}\) near the origin with \(s(0)=0\),
\(\nabla s(0)=0\), and positive definite quadratic jet
\(g_{ij}=\partial_i\partial_j s(0)\). Assume all jets of order \(4,6,\ldots,2k-2\)
vanish. Let \(P_{i_1\cdots i_{2k}}=\partial_{i_1}\cdots\partial_{i_{2k}}s(0)\) be the
fully symmetric \(2k\)-jet tensor. Then for \(\varepsilon>0\) sufficiently small,
\begin{equation}
V(\varepsilon)
= \frac{\omega_n}{n}\,\frac{(2\varepsilon)^{n/2}}{\sqrt{\det g}}
\left(1
  - \frac{\varepsilon^{k-1}}{2\,k!\,\prod_{j=1}^{k-1}(n+2j)}\,
    \mathrm{Tr}_g^k(P)
  + O(\varepsilon^k) + O(\|P\|^2)\right),
\label{eq:general-2k-variation}
\end{equation}
where
\(\mathrm{Tr}_g^k(P):=g^{i_1 i_2}g^{i_3 i_4}\cdots g^{i_{2k-1}i_{2k}}
P_{i_1 i_2\cdots i_{2k}}\) is the \(k\)-fold trace with respect to \(g\).
\end{theorem}

\begin{proof}
By a linear change of variables set \(g=I\) (at the cost of a Jacobian
\(1/\sqrt{\det g}\)). Write \(z=rn\) with \(n\in S^{n-1}\). The jet expansion gives
\(s(rn)=\tfrac12 r^2+\tfrac{1}{(2k)!}r^{2k}P(n,\ldots,n)\). Setting \(s=\varepsilon\)
and solving perturbatively yields
\[
r^2 = 2\varepsilon\!\left(1 - \frac{2^{k-1}\varepsilon^{k-1}}{(2k)!}\,P(n,\ldots,n)
  + O(\varepsilon^k) + O(\|P\|^2)\right).
\]
Hence
\(r^n=(2\varepsilon)^{n/2}(1-\frac{n\,2^{k-1}\varepsilon^{k-1}}{2(2k)!}\,P(n,\ldots,n)
+\cdots)\), and integrating over \(S^{n-1}\) using the \(O(n)\)-invariant \(2k\)-th moment
\begin{equation}
\int_{S^{n-1}} P(n,\ldots,n)\,d\Omega
= \frac{(2k-1)!!\;\omega_n}{\prod_{j=0}^{k-1}(n+2j)}\;\mathrm{Tr}^k(P)
\label{eq:sphere-2k-moment}
\end{equation}
(which generalises~\eqref{eq:sphere-4-moment-Sn}) gives
\eqref{eq:general-2k-variation} after simplification using
\((2k-1)!!/(2k)!=1/(2^k k!)\).
\end{proof}

\begin{remark}[Recovering the quartic and sextic cases]
For \(k=2\) (quartic) and \(n=2\) the coefficient becomes
\(\varepsilon/(2\cdot 2\cdot 4) = \varepsilon/16\), and at \(\varepsilon=1/2\) this gives
\(1/32\) times the double trace, recovering
Theorem~\ref{thm:invariant-first-variation}. For \(k=3\) (sextic) and \(n=2\) the
coefficient becomes \(\varepsilon^2/(2\cdot 6\cdot 4\cdot 6) = \varepsilon^2/288\), and
at \(\varepsilon=1/2\) this gives \(1/1152\) times the triple trace, recovering
\eqref{eq:sextic-area-normalised}.
\end{remark}

\begin{remark}[Angular-integral ratios at order \(2k\)]
\label{rem:angular-ratios}
In \(n=2\) dimensions, the ratio of the cross angular integral
\[
\int_0^{2\pi}\cos^{2k-2}\theta\sin^2\theta\,d\theta
\]
to the diagonal integral
\[
\int_0^{2\pi}\cos^{2k}\theta\,d\theta
\]
is \(1/(2k-1)\). For \(k=2\) this gives \(1/3\); for \(k=3\) it gives \(1/5\). This
ratio is the geometric origin of the decreasing per-component weight of
cross-coupling terms at higher jet orders.
\end{remark}

%------------------------------------------------------------------
\section{Universality at All Even Orders}
\label{sec:universality-all}
%------------------------------------------------------------------

We now resolve the conjecture stated in Section~\ref{sec:hierarchy}: normalisability of
\(\exp(-s_{2k})\) forces a positive first-order correction at every even order \(2k\).
The argument is a direct consequence of spherical averaging and requires no case analysis.

\begin{theorem}[Universality at all even orders]
\label{thm:universality-all-orders}
Let \(H:\mathbb{R}^n\to\mathbb{R}\) be a homogeneous polynomial of degree \(2k\)
(\(k\ge 1\)) that is non-negative: \(H(z)\ge 0\) for all \(z\in\mathbb{R}^n\), and not
identically zero. Let \(P_{i_1\cdots i_{2k}}\) be the associated fully symmetric tensor
(so \(H(z)=P_{i_1\cdots i_{2k}}z^{i_1}\cdots z^{i_{2k}}\)). Then for any positive
definite metric \(g\),
\begin{equation}
\mathrm{Tr}_g^k(P) > 0.
\label{eq:positive-multi-trace}
\end{equation}
Moreover, if \(H\ge 0\) and \(\mathrm{Tr}_g^k(P)=0\), then \(H\equiv 0\) and \(P=0\).
\end{theorem}

\begin{proof}
By a linear change of coordinates set \(g=I\). The spherical \(2k\)-th moment formula
\eqref{eq:sphere-2k-moment} gives
\[
\int_{S^{n-1}} H(n)\,d\Omega
= \frac{(2k-1)!!\;\omega_n}{\prod_{j=0}^{k-1}(n+2j)}\;\mathrm{Tr}^k(P),
\]
where the prefactor is strictly positive. Since \(H\) is continuous, non-negative, and
not identically zero on the compact set \(S^{n-1}\), its integral is strictly positive.
Therefore \(\mathrm{Tr}^k(P) > 0\).

For the rigidity statement: if \(\mathrm{Tr}^k(P)=0\) and \(H\ge 0\), then
\(\int_{S^{n-1}} H(n)\,d\Omega = 0\), which forces \(H\equiv 0\) on \(S^{n-1}\) by
continuity and non-negativity. Since \(H\) is homogeneous, \(H\equiv 0\) on all of
\(\mathbb{R}^n\). By the polarisation identity for symmetric tensors, \(P=0\).
\end{proof}

\begin{corollary}[Resolution of the universality conjecture]
\label{cor:universality-resolved}
At every even order \(2k\ge 4\), let
\(s_{2k}(z)=\tfrac12 g_{ij}z^iz^j + \tfrac{1}{(2k)!}P_{i_1\cdots i_{2k}}
z^{i_1}\cdots z^{i_{2k}}\) be a seam whose model density \(\exp(-s_{2k})\) is
normalisable and whose \(2k\)-jet does not vanish. Then the first-order correction
coefficient \(\mathrm{Tr}_g^k(P)\) is strictly positive, and consequently sublevel-set
volumes are strictly smaller than those of the quadratic seam.
\end{corollary}

\begin{proof}
Normalisability of \(\exp(-s_{2k})\) requires the homogeneous degree-\(2k\) part
\[
H(z)=\tfrac{1}{(2k)!}P_{i_1\cdots i_{2k}}z^{i_1}\cdots z^{i_{2k}}
\]
to be non-negative for all \(z\): if \(H(z_0)<0\) for some direction \(z_0\), then
\(s_{2k}(rz_0)\to -\infty\) as \(r\to\infty\), so \(\exp(-s_{2k})\) is not integrable.
Apply Theorem~\ref{thm:universality-all-orders}.
\end{proof}

\begin{remark}[Relation to the quartic proof]
At quartic order (\(k=2\), \(n=2\)) the normalisability constraint reduces to
\(c\ge-\sqrt{ab}\), and the positivity of \(3a+3b+2c\) was established in
Theorem~\ref{thm:universality} via the AM-GM inequality. That argument is a special case
of the present proof: the AM-GM step verifies, by direct algebra, that
\(\int_{S^1}Q(\cos\theta,\sin\theta)\,d\theta > 0\) whenever the quartic form \(Q\ge 0\)
is non-trivial. The spherical-averaging argument replaces all such case-specific algebra
with a single measure-theoretic step.
\end{remark}
...
%------------------------------------------------------------------
\section{A Concrete Physical Example: Weak Kerr-Type Nonlinearity}
%------------------------------------------------------------------

To ground the framework, consider a single-mode oscillator with a weak isotropic
Kerr-type nonlinearity, modeled at the classical level by
\[
H(x,p)=\frac{p^2}{2m}+\frac{m\omega^2 x^2}{2}+\frac{\kappa}{4}\,(p^2+m^2\omega^2 x^2)^2,
\]
with \(\kappa>0\). A simple toy non-Gaussian phase-space density is the thermal
ansatz \(\rho\propto\exp(-\beta H)\), or equivalently a seam
\(s=\beta H\). In scaled variables \(u=x/\sigma_x\), \(v=p/\sigma_p\) with
\(\sigma_x^2=1/(\beta m\omega^2)\) and \(\sigma_p^2=m/\beta\), this takes the
quartic seam form
\[
s_4(u,v)=\frac{u^2+v^2}{2}+\frac{\lambda}{4}\bigl(u^4+v^4+2u^2v^2\bigr),
\qquad \lambda=\frac{\kappa m^2}{\beta},
\]
with the concrete coefficients
\[
a=1,\qquad b=1,\qquad c=1.
\]
This is the rotationally symmetric case \(s_4=\tfrac{1}{2}(u^2+v^2)
+\tfrac{\lambda}{4}(u^2+v^2)^2\), which is normalisable for all \(\lambda>0\)
and yields a positive quartic correction \(\mathcal{W}_4=\lambda(3+3+2)/16
=\lambda/2\). The coefficients \(a,b,c\) thus have a direct physical
interpretation as the relative strengths of the quartic self- and cross-kurtosis
induced by the Kerr term.
...
\item The framework extends beyond the quartic: a general first-variation formula
(Theorem~\ref{thm:general-2k-variation}) shows that the first-order volume correction
from any even \(2k\)-jet depends only on its \(k\)-fold trace. The ratio of cross to
diagonal angular integrals at order \(2k\) is \(1/(2k-1)\) in \(d=1\) (giving \(1/3\)
at quartic order and \(1/5\) at sextic order), a universal spherical-geometry invariant.

\item Universality holds at all even orders
(Theorem~\ref{thm:universality-all-orders}): normalisability of \(\exp(-s_{2k})\)
forces the \(k\)-fold trace --- and hence the first-order volume correction --- to be
strictly positive for any non-trivial \(2k\)-jet. The proof is a single
measure-theoretic step that replaces the quartic-specific AM-GM argument.
\end{enumerate}

The immediate computational payoffs (convex inverse design and visual diagnostics) are
ready for implementation using \(\mathcal{W}_4\) (or its higher-order analogues) rather
than the diagonal scalar. The multi-mode extension confirms that the \(1:3\) ratio at
quartic order is dimension-independent; at higher jet orders the corresponding ratios
follow the pattern \(1/(2k-1)\) (Remark~\ref{rem:angular-ratios}).
...
\textbf{Lars R\"onnb\"ack}: Conceptualization (lead); Formal analysis (lead); Investigation (lead); Methodology (lead); Writing -- original draft (lead); Writing -- review \& editing (lead).

\noindent\textbf{Declaration of generative AI and AI-assisted technologies.}
During the preparation of this work the author used Grok 4.2, GPT-5.2, Gemini 3.1 Pro, and Claude Sonnet 4.6 in order to draft early versions of the manuscript, brainstorm and outline the revised manuscript, polish the exposition, and respond to reviewer feedback. After using these tools/services, the author reviewed and edited the content as needed and takes full responsibility for the content of the published article.

\section*{Data Availability}
This article presents theoretical results only; no datasets were generated or analysed. The data that support the findings of this study are available within the article itself.
