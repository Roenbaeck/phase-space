...
In particular, consider the quadratic seam
\[
...
an ellipse with Lebesgue area \(\pi\sigma_x\sigma_p\)
(for general \(d\), the volume is \(\frac{\pi^d}{d!}\prod_i\sigma_{x_i}\sigma_{p_i}\)).
...
We extend the framework from the quartic to the sextic (6-th order) jet, computing
explicit area corrections in \(d=1\), and prove a general first-variation formula for
arbitrary even-order \(2k\)-jets in \(\mathbb{R}^n\). We then establish universality of
positive corrections at all even orders: for any non-trivial normalisable \(2k\)-jet
perturbation, the \(k\)-fold trace of the jet tensor is strictly positive, so that
non-Gaussianity always reduces sublevel-set volumes to first order. The proof is a
single measure-theoretic step (spherical averaging of a non-negative function) and
subsumes the quartic AM-GM argument as a special case.
